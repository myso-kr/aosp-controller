...
export async function ControllerDeviceChrome(adb, serial, rooted) {
  ...
  console.info(`${JSON.stringify(forwards)}`);
  const foward = _.find(forwards, { remote: 'localabstract:chrome_devtools_remote' });
  const port = parseInt(_.get(foward, 'local', "").replace(/[^\d]/g, '') || _.random(9223, 9323));
