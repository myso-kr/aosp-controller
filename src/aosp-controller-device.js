        ...
        try {
          ...
          switch(action++ % 3) {
            ...
            case 2:
              ...
              await chromeDeviceEmulationGoBack({ match: /search\.naver\.com/g });
            break;
          }
        } catch(e) {
          reject(e);
...
export async function ControllerDeviceChrome(adb, serial, rooted) {
  ...
  if(rooted) {
    ...
    await adb.shellWait(serial, `su -c 'echo "${timestamp.minutes()} ${timestamp.hours()} * * * reboot" > /sdcard/android/crontabs/root' root`);
